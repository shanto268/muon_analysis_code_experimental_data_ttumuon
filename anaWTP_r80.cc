//
//  c++  `root-config --cflags` -o anabin anaWTP_v03f1.cc   `root-config --glibs`
//  ./anabin  1 2 3 4 5 6 7
//  (1) inputFileName
//  (2) outputFileName 
//  (3) theta
//  (4) phi
//  (5) time window  (default 1)
//  (6) cluster size max  (default 2)
//  (7) max events to analyze
//
//  for example,
...
sc8Input::sc8Input(string inname) {
  ...
  adcThreshold[0][0] = 10;  // #Layer 0, BB 0
  adcThreshold[1][0] = 10;
  adcThreshold[2][0] = 10;
  adcThreshold[3][0] = 10;
  adcThreshold[4][0] = 10;
  ...
  adcThreshold[0][1] = 10;  // #Layer 0, BB 1
  adcThreshold[1][1] = 10;
  adcThreshold[2][1] = 10;
  adcThreshold[3][1] = 10;
  adcThreshold[4][1] = 10;
  adcThreshold[5][1] = 255;

  adcThreshold[0][2] = 10;  // #Layer 1, BB 0
  adcThreshold[1][2] = 30;
  adcThreshold[2][2] = 10;
  adcThreshold[3][2] = 10;
  adcThreshold[4][2] = 10;
  ...
  adcThreshold[0][3] = 255; // #Layer 1, BB 1
  adcThreshold[1][3] = 10;
  adcThreshold[2][3] = 10;
  adcThreshold[3][3] = 10;
  adcThreshold[4][3] = 255;
  adcThreshold[5][3] = 255;

  adcThreshold[0][4] = 10;  // #Layer 2, BB 0
  adcThreshold[1][4] = 30;
  adcThreshold[2][4] = 30;
  adcThreshold[3][4] = 30;
  adcThreshold[4][4] = 30;
  ...
  adcThreshold[0][5] = 30;  // #Layer 2, BB 1
  adcThreshold[1][5] = 30;
  adcThreshold[2][5] = 30;
  adcThreshold[3][5] = 30;
  adcThreshold[4][5] = 255;
  adcThreshold[5][5] = 255;

  adcThreshold[0][6] = 30;  // #Layer 3 BB 0
  adcThreshold[1][6] = 10;
  adcThreshold[2][6] = 10;
  adcThreshold[3][6] = 10;
  adcThreshold[4][6] = 10;
  ...
  adcThreshold[0][7] = 10;  // #Layer 3, BB 1
  adcThreshold[1][7] = 10;
  adcThreshold[2][7] = 10;
  adcThreshold[3][7] = 10;
  adcThreshold[4][7] = 255;
  adcThreshold[5][7] = 255;
...
int sc8Input::readEvent(sc8Event & evt){
   ...
   if(infile.is_open()) {
      ...
      while(getline(infile,line)) {
         ...
         int iskip=0;
         if(line.size()>20) {
            std::istringstream isx(line);
            int j;
            string s;
            isx>>j;  if(j==0) iskip=1;  // run number
            isx>>j;  if(j==0) iskip=1;  // run number
            isx>>s;  // pc date
            isx>>s;  // pc time
            int aaa[4];
            int bbb[28];
            for(int i=0; i<4; i++) { isx>>aaa[i]; if(aaa[i]==299) iskip=1;}
            for(int i=0; i<28; i++) {isx>>bbb[i]; if(bbb[i]==299) iskip=1;}
            if(aaa[3] != 222) iskip=1;
            if(bbb[1] != 11) iskip=1;
            if(isx.fail()) iskip=1;   // some words are missing.
         }
         if(iskip==1) continue;
         ...
         e.pcRunNumber.push_back(pcrunnumber);
         int pcevent;
         iss>>pcevent;
         ncounts=ncounts+1;
         ...
         if(nhits>0) {
           ...
           for (int i=0; i<nhits; i++) {
              ...
              h.adc[0]=b[k+2];
              h.adc[1]=b[k+3];
              bool adcTh=(h.adc[1]-h.adc[0]) > adcThreshold[h.ch][node-1];
              //  addtional hit masking...
              if(h.time>0 && adcTh) {
                // check the validity of values before saving it...
                if((h.layer>=0 && h.layer<=3) && (h.bb==0 || h.bb==1) && (h.ch>=0 && h.ch<=5)) {
                  // - no mask if(!(node==4 && h.ch == 0)) {     // masking a channel
                      e.hits.push_back(h);
                  // }
                }
              }
...
void HistSet1::init(TFile* _fout, string _dirName) {
   ...
   histo1D["N1nhits"]=new TH1D("N1nhits","Node 1- number of hits",20,0.,20.);
   histo1D["N2nhits"]=new TH1D("N2nhits","Node 2- number of hits",20,0.,20.);
   histo1D["N3nhits"]=new TH1D("N3nhits","Node 3- number of hits",20,0.,20.);
   histo1D["N4nhits"]=new TH1D("N4nhits","Node 4- number of hits",20,0.,20.);
   histo1D["N5nhits"]=new TH1D("N5nhits","Node 5- number of hits",20,0.,20.);
   histo1D["N6nhits"]=new TH1D("N6nhits","Node 6- number of hits",20,0.,20.);
   histo1D["N7nhits"]=new TH1D("N7nhits","Node 7- number of hits",20,0.,20.);
   histo1D["N8nhits"]=new TH1D("N8nhits","Node 8- number of hits",20,0.,20.);
...
void HistSet1::analyze(sc8Event& evt) {
   ...
   int nperNode[8]={0,0,0,0,0,0,0,0};
   for (int i=0; i<nhits; i++) {

      int nodeNumber=e.hits[i].layer*2+e.hits[i].bb;
      nperNode[nodeNumber]=nperNode[nodeNumber]+1;
   ...
   for(int i=0; i<8; i++) {
      int j=i+1;
      string hname="N"+to_string(j)+"nhits";
      histo1D[hname]->Fill(nperNode[i]);
   }
...
class HistWaterTower{
   ...
   private:
      ...
      double lastEventNumber;  // to measure the duration of this run
...
void HistWaterTower::init(TFile* _fout, string _dirName) {
   ...
   double xmin=-25.0;
   double xmax=25.0;
   histo1D["lastEvent"]=new TH1D("lastEvent","last Event number",5,0.0,5.0);
   histo1D["muXproj"]=new TH1D("muXproj","Muon X-projection",nbins,xmin,xmax);
   histo1D["muYproj"]=new TH1D("muYproj","Muon Y-projection",nbins,xmin,xmax);
   histo1D["muXpr2"]=new TH1D("muXpr2","Muon X-proj (in y band)",nbins,xmin,xmax);
   histo1D["muYpr2"]=new TH1D("muYpr2","Muon Y-proj (in x band)",nbins,xmin,xmax);
   histo1D["muXpr3"]=new TH1D("muXpr3","Muon X-proj (in y side-band)",nbins,xmin,xmax);
   histo1D["muYpr3"]=new TH1D("muYpr3","Muon Y-proj (in x band -10 to 0)",nbins,xmin,xmax);
   histo2D["muXYproj"]=new TH2D("muXYproj","Muon XY-projection",nbins,xmin,xmax,nbins,xmin,xmax);
}
...
//  similar to Y.
   ...
   tdir->cd();
  lastEventNumber=evt.getRawData().pcEventNumber[0];
...
// ==========================================================================
void HistWaterTower::endjob() {
   tdir->cd();
   histo1D["lastEvent"]->Fill(1.0,lastEventNumber);
}
// ==========================================================================
// ==========================================================================
class sc8Analyzer{
...
void sc8Analyzer::endjob(){
  ...
  _histwatertower.endjob();  // to fill last event number
  fout->Write();
  fout->Close();
...
int main( int argc, char **argv ) {
   ...
   string indir="/Users/skunori/muonsc8/daq/test/20190901/v5/rawFiles/";
   string outdir="/Users/skunori/muonsc8/daq/test/20190901/v5/histFiles/";
   ...
   for(int i=0; i<nmaxRead; i++) {
      ...
      if((i%10000)==0) cout<<"event i="<<i<<endl;
